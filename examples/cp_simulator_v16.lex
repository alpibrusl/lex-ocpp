...
fn config_from_env() -> [env] Config {
  let cp := match env.get("CP_ID") {
    Some(v) => v,
    None => default_cp_id(),
  }
  let it := match env.get("ID_TAG") {
    Some(v) => v,
    None => default_id_tag(),
  }
  { cp_id: cp, id_tag: it }
...
fn start_call(mid :: Str, id_tag :: Str) -> Str {
  msg.encode(msg.new_call(mid, "StartTransaction", JObj([("connectorId", JInt(connector_id())), ("idTag", JStr(id_tag)), ("meterStart", JInt(meter_start_wh())), ("timestamp", JStr(ts_start()))])))
}

fn meter_call(mid :: Str, tx_id :: Int) -> Str {
  msg.encode(msg.new_call(mid, "MeterValues", JObj([("connectorId", JInt(connector_id())), ("transactionId", JInt(tx_id)), ("meterValue", JList([JObj([("timestamp", JStr(ts_sample())), ("sampledValue", JList([JObj([("value", JStr(int.to_str(sample_wh())))])]))])]))])))
}
...
fn stop_call(mid :: Str, tx_id :: Int) -> Str {
  msg.encode(msg.new_call(mid, "StopTransaction", JObj([("transactionId", JInt(tx_id)), ("meterStop", JInt(meter_stop_wh())), ("timestamp", JStr(ts_stop())), ("reason", JStr(en.reason_local()))])))
}
...
fn step_meter(tx :: Int) -> Str {
  str.concat("meter:tx", int_str(tx))
}

fn step_heartbeat(tx :: Int) -> Str {
  str.concat("heartbeat:tx", int_str(tx))
}

fn step_stop(tx :: Int) -> Str {
  str.concat("stop:tx", int_str(tx))
}
    ...
    if mid == step_status_init() {
      ...
      if mid == step_auth() {
        if authorize_accepted(payload) {
          ...
          let __lex_discard_6 := io.print(str.concat("× Authorize rejected; session aborts. idTagInfo: ", jv.stringify(payload)))
          WsNoOp
        }
      } else {
        if mid == step_start() {
...
fn on_result_tx(mid :: Str) -> [io] WsAction {
  ...
  if starts_with(mid, "meter:tx") {
    ...
    if starts_with(mid, "heartbeat:tx") {
      emit("StopTransaction", stop_call(step_stop(tx), tx))
    } else {
      if starts_with(mid, "stop:tx") {
...
# ---- Entry point -------------------------------------------------
fn make_on_message(id_tag :: Str) -> (WsMessage) -> [io] WsAction {
...
fn main() -> [net, io, env] Nil {
  ...
  let __lex_discard_11 := io.print(str.concat("=== cp_simulator_v16 [", str.concat(cfg.cp_id, str.concat("] → ", url))))
  let __lex_discard_12 := io.print(str.concat("    subprotocol: ", cp.version_v16()))
  let __lex_discard_13 := io.print(str.concat("    id_tag: ", cfg.id_tag))

...
fn default_id_token() -> Str {
  "USER-001"
}

type Config = { cp_id :: Str, id_token :: Str }

fn config_from_env() -> [env] Config {
  let cp_v := match env.get("CP_ID") {
    Some(v) => v,
    None => default_cp_id(),
  }
  let id_v := match env.get("ID_TOKEN") {
    Some(v) => v,
    None => default_id_token(),
  }
...
fn id_token_type() -> Str {
  "ISO14443"
}

# ---- Frame builders ----------------------------------------------
fn boot_call(mid :: Str) -> Str {
  msg.encode(msg.new_call(mid, "BootNotification", JObj([("chargingStation", JObj([("model", JStr(model())), ("vendorName", JStr(vendor()))])), ("reason", JStr("PowerUp"))])))
}

fn status_call(mid :: Str, status :: Str) -> Str {
  msg.encode(msg.new_call(mid, "StatusNotification", JObj([("timestamp", JStr(ts_boot())), ("connectorStatus", JStr(status)), ("evseId", JInt(evse_id())), ("connectorId", JInt(connector_id()))])))
}

fn authorize_call(mid :: Str, id_token :: Str) -> Str {
  msg.encode(msg.new_call(mid, "Authorize", JObj([("idToken", JObj([("idToken", JStr(id_token)), ("type", JStr(id_token_type()))]))])))
}
...
fn meter_value_obj(ts :: Str, wh :: Int) -> jv.Json {
  JObj([("timestamp", JStr(ts)), ("sampledValue", JList([JObj([("value", JFloat(int.to_float(wh)))])]))])
}

fn tx_event_call(mid :: Str, event_type :: Str, seq_no :: Int, charging_state :: Str, stopped_reason :: Str, meter_ts :: Str, meter_wh :: Int, id_token :: Str) -> Str {
  let tx_info_base := [("transactionId", JStr(txn_id())), ("chargingState", JStr(charging_state))]
  let tx_info := if stopped_reason == "" {
    tx_info_base
  } else {
    list.concat(tx_info_base, [("stoppedReason", JStr(stopped_reason))])
  }
  let id_token_obj := JObj([("idToken", JStr(id_token)), ("type", JStr(id_token_type()))])
  msg.encode(msg.new_call(mid, "TransactionEvent", JObj([("eventType", JStr(event_type)), ("timestamp", JStr(meter_ts)), ("seqNo", JInt(seq_no)), ("transactionInfo", JObj(tx_info)), ("idToken", id_token_obj), ("evse", JObj([("id", JInt(evse_id())), ("connectorId", JInt(connector_id()))])), ("meterValue", JList([meter_value_obj(meter_ts, meter_wh)]))])))
}
...
fn stub_call_result(mid :: Str) -> Str {
  msg.encode(msg.new_call_result(mid, JObj([("status", JStr(en.gen_accepted()))])))
}
...
fn step_tx_started() -> Str {
  "tx:started"
}

fn step_tx_updated() -> Str {
  "tx:updated"
}

fn step_heartbeat() -> Str {
  "heartbeat"
}

fn step_tx_ended() -> Str {
  "tx:ended"
}
...
fn on_open() -> [io] WsAction {
  let __lex_discard_1 := io.print("-> open: sending BootNotification (PowerUp)")
...
fn on_text(raw :: Str, id_token :: Str) -> [io] WsAction {
  ...
  match msg.parse(raw) {
    ...
    Ok(FrameCall(c)) => {
      let __lex_discard_5 := io.print(str.concat("  (stub-replying to CSMS Call: ", c.action))
...
fn on_result(mid :: Str, payload :: jv.Json, id_token :: Str) -> [io] WsAction {
  if mid == step_boot() {
    emit("StatusNotification(Available)", status_call(step_status_init(), en.cs_available()))
  } else {
    if mid == step_status_init() {
      ...
      if mid == step_auth() {
        if authorize_accepted(payload) {
          emit("TransactionEvent(Started)", tx_event_call(step_tx_started(), "Started", 0, en.ch_charging(), "", ts_tx_open(), meter_start_wh(), id_token))
        } else {
          let __lex_discard_6 := io.print(str.concat("× Authorize rejected; aborting. idTokenInfo: ", jv.stringify(payload)))
          WsNoOp
        }
      } else {
        if mid == step_tx_started() {
          emit("TransactionEvent(Updated)", tx_event_call(step_tx_updated(), "Updated", 1, en.ch_charging(), "", ts_tx_mid(), meter_mid_wh(), id_token))
        } else {
          if mid == step_tx_updated() {
            emit("Heartbeat", heartbeat_call(step_heartbeat()))
          } else {
            if mid == step_heartbeat() {
              emit("TransactionEvent(Ended)", tx_event_call(step_tx_ended(), "Ended", 2, en.ch_idle(), "Local", ts_tx_end(), meter_stop_wh(), id_token))
            } else {
              if mid == step_tx_ended() {
                emit("StatusNotification(Available)", status_call(step_status_final(), en.cs_available()))
              } else {
                if mid == step_status_final() {
                  let __lex_discard_7 := io.print("=== session complete; idling (Ctrl-C to exit) ===")
                  WsNoOp
                } else {
                  let __lex_discard_8 := io.print(str.concat("? unknown step id: ", mid))
                  WsNoOp
                }
              }
            }
...
fn emit(label :: Str, frame :: Str) -> [io] WsAction {
  let __lex_discard_9 := io.print(str.concat("-> ", str.concat(label, str.concat(": ", frame))))
  WsSend(frame)
}
...
# ---- Entry point ------------------------------------------------
fn make_on_message(id_token :: Str) -> (WsMessage) -> [io] WsAction {
  fn (m :: WsMessage) -> [io] WsAction {
    match m {
      WsText(raw) => on_text(raw, id_token),
      WsClose => {
        let __lex_discard_10 := io.print("<- server closed connection")
        WsNoOp
      },
      WsPing => WsNoOp,
      WsBinary(_) => WsNoOp,
...
fn main() -> [net, io, env] Nil {
  ...
  let __lex_discard_11 := io.print(str.concat("=== cp_simulator_v21 [", str.concat(cfg.cp_id, str.concat("] -> ", url))))
  let __lex_discard_12 := io.print(str.concat("    subprotocol: ", cp.version_v21()))
  let __lex_discard_13 := io.print(str.concat("    id_token: ", cfg.id_token))

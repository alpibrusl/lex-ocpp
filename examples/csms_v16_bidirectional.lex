...
fn default_db_path() -> Str { "/tmp/csms.db" }
...
fn seed_allowed_tags(db :: Db) -> [sql] Result[Unit, SqlError] {
  match sql.exec(db,
    ...
    [PStr("USER-001"), PStr("default demo card")])
  {
    Err(e) => Err(e),
    Ok(_)  => Ok(()),
  }
...
fn on_boot(
  ...
  let _ := io.print(str.concat("  [", str.concat(cp_id,
            str.concat("] BootNotification vendor=",
  ...
  match sql.exec(db,
    "INSERT INTO charge_points (cp_id, vendor, model, last_boot)
     ...
     ON CONFLICT(cp_id) DO UPDATE SET vendor=excluded.vendor,
       model=excluded.model, last_boot=excluded.last_boot",
...
fn tag_is_allowed(db :: Db, id_tag :: Str) -> [sql] Bool {
  let res :: Result[List[{ id_tag :: Str }], SqlError] :=
    sql.query(db, "SELECT id_tag FROM allowed_tags WHERE id_tag = ?", [PStr(id_tag)])
  match res {
    Err(_)   => false,
    ...
    Ok(_)  => match last_insert_rowid(db) {
      ...
      Ok(tx_id) => {
        ...
        HOk(JObj([
          ...
          ("idTagInfo",     JObj([("status", JStr(status))])),
        ]))
      },
    },
  }
...
fn last_insert_rowid(db :: Db) -> [sql] Result[Int, SqlError] {
  let res :: Result[List[{ id :: Int }], SqlError] :=
    sql.query(db, "SELECT last_insert_rowid() AS id", [])
  match res {
    Err(e)   => Err(e),
...
fn on_meter_values(
  ...
  let ts       := sample.ts
  let _ := io.print(str.concat("  [", str.concat(cp_id,
            str.concat("] MeterValues conn=", str.concat(int.to_str(conn_id),
              str.concat(" tx=", str.concat(int.to_str(tx_id),
                str.concat(" wh=", int.to_str(value_wh)))))))))
  let tx_param := if tx_id == 0 { PNull } else { PInt(tx_id) }
  match sql.exec(db,
    "INSERT INTO meter_values (cp_id, transaction_id, connector_id, value_wh, ts)
       VALUES (?, ?, ?, ?, ?)",
    [PStr(cp_id), tx_param, PInt(conn_id), PInt(value_wh), PStr(ts)])
  {
    Err(e) => HErr(oe.internal_err(e.message)),
    Ok(_)  => HOk(JObj([])),
  }
...
fn on_stop_tx(
  ...
  let _ := io.print(str.concat("  [", str.concat(cp_id,
            str.concat("] StopTransaction tx=",
              str.concat(int.to_str(tx_id), str.concat(" reason=", reason))))))
  match sql.exec(db,
...
fn dequeue_pending(db :: Db, cp_id :: Str) -> [sql] Option[PendingRow] {
  let res :: Result[List[PendingRow], SqlError] :=
    sql.query(db,
      "SELECT id, message_id, action, frame FROM pending_commands
         WHERE cp_id=? ORDER BY id LIMIT 1",
      [PStr(cp_id)])
  match res {
    Err(_)   => None,
...
fn mark_sent(db :: Db, row :: PendingRow, cp_id :: Str, now :: Str) -> [sql] Unit {
  let _ := sql.exec(db,
    "INSERT INTO cs_to_cp_log
       (cp_id, message_id, action, sent_frame, sent_at)
       VALUES (?, ?, ?, ?, ?)",
    [PStr(cp_id), PStr(row.message_id), PStr(row.action),
     PStr(row.frame), PStr(now)])
  let _ := sql.exec(db,
...
fn on_heartbeat(
  db :: Db, cp_id :: Str, _payload :: jv.Json, now :: Str
) -> [io, sql] Str {
  match dequeue_pending(db, cp_id) {
    ...
    Some(row) => {
      let _ := io.print(str.concat("  [", str.concat(cp_id,
                str.concat("] → sending pending command ", row.action))))
      let _ := mark_sent(db, row, cp_id, now)
      row.frame
    },
  }
}
...
fn on_call_result(
  db :: Db, cp_id :: Str, message_id :: Str, payload :: jv.Json, now :: Str
) -> [io, sql] Unit {
  ...
  let _ := io.print(str.concat("  [", str.concat(cp_id,
            str.concat("] CallResult msg=", str.concat(message_id,
              str.concat(" status=", status))))))
  let _ := sql.exec(db,
    "UPDATE cs_to_cp_log
       SET status=?, response_json=?, responded_at=?
       WHERE cp_id=? AND message_id=?",
    [PStr(status), PStr(payload_str), PStr(now), PStr(cp_id), PStr(message_id)])
  ()
}
...
fn handle_raw(db :: Db, cp_id :: Str, raw :: Str) -> [io, time, sql] Str {
  ...
  match msg.parse(raw) {
    ...
    Ok(FrameCallResult(r)) => {
      let _ := on_call_result(db, cp_id, r.message_id, r.payload, now)
      ""
    },
    Ok(FrameCallError(er)) => {
      let _ := io.print(str.concat("  [", str.concat(cp_id,
                str.concat("] inbound CallError: ", er.description))))
      ""
    },
  }
}
  ...
  fn (conn :: WsConn, m :: WsMessage) -> [io, time, sql] WsAction {
    ...
    match m {
      WsText(raw) => {
        ...
        if out == "" { WsNoOp } else { WsSend(out) }
      },
      WsClose     => {
        let _ := io.print(str.concat("  [", str.concat(cp_id, "] connection closed")))
        WsNoOp
      },
      WsPing      => WsNoOp,
      WsBinary(_) => WsNoOp,
...
fn main() -> [net, io, time, sql, fs_write, env] Nil {
  let path := db_path()
  let _ := io.print("CSMS v1.6 (bidirectional)  ws://localhost:9000/ocpp/<cp_id>")
  ...
  match sql.open(path) {
    ...
    Ok(db) => match init_db(db) {
      ...
      Ok(_)  => {
        let _ := io.print("  schema ready; queue commands via pending_commands table")

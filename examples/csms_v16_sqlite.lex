...
fn on_start_tx(
    ...
    [PStr(cp_id), PInt(conn_id), PStr(id_tag), PInt(m_start), PStr(ts)])
  {
    ...
    Ok(_)  => match last_insert_rowid(db) {
      ...
      Ok(tx_id) => {
        let _ := io.print(str.concat("  [", str.concat(cp_id,
                  str.concat("] StartTransaction → tx=",
                    int.to_str(tx_id)))))
...
fn on_meter_values(
  ...
  let tx_id   := match tx_id_o { Some(JInt(n)) => n, _ => 0 }
...
fn handle_raw(
  ...
  match msg.parse(raw) {
    ...
    Ok(FrameCallResult(_)) => "",   # CP shouldn't be sending these
    Ok(FrameCallError(e))  => {
      let _ := io.print(str.concat("  [", str.concat(cp_id,
...
fn main() -> [net, io, time, sql, fs_write, env] Nil {
  let path := db_path()
  let _ := io.print("CSMS v1.6 (sqlite)  ws://localhost:9000/ocpp/<cp_id>")
  ...
  match sql.open(path) {
    ...
    Ok(db) => match init_db(db) {
      ...
      Ok(_)  => {
        let _ := io.print("  schema ready; pre-seeded allowed_tags with USER-001")

...
fn diagnostics_status_notification() -> Str {
...
fn get_configuration() -> Str {
  "GetConfiguration"
}

fn get_diagnostics() -> Str {
  "GetDiagnostics"
}
...
fn remote_start_transaction() -> Str {
  "RemoteStartTransaction"
}

fn remote_stop_transaction() -> Str {
...
fn cp_to_cs() -> List[Str] {
  [authorize(), boot_notification(), data_transfer(), diagnostics_status_notification(), firmware_status_notification(), heartbeat(), meter_values(), start_transaction(), status_notification(), stop_transaction()]
}

fn cs_to_cp() -> List[Str] {
  [cancel_reservation(), change_availability(), change_configuration(), clear_cache(), clear_charging_profile(), data_transfer(), get_composite_schedule(), get_configuration(), get_diagnostics(), get_local_list_version(), remote_start_transaction(), remote_stop_transaction(), reserve_now(), reset(), send_local_list(), set_charging_profile(), trigger_message(), unlock_connector(), update_firmware()]
}

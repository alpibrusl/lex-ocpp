...
fn sampled_value(value :: Str) -> SampledValue {
  {
    value: value, context: None, format: None,
    measurand: None, phase: None, location: None, unit: None,
  }
}
...
fn meter_value_to_json(mv :: MeterValue) -> jv.Json {
  JObj([
    ("timestamp",    JStr(mv.timestamp)),
    ("sampledValue", JList(list.map(mv.sampled_value, sampled_value_to_json))),
  ])
}
...
fn charging_schedule_period_to_json(p :: ChargingSchedulePeriod) -> jv.Json {
  let base := [
    ("startPeriod", JInt(p.start_period)),
    ("limit",       JFloat(p.limit)),
  ]
...
type ChargingSchedule = {
  ...
  min_charging_rate         :: Option[Float],
}
...
fn charging_schedule_to_json(cs :: ChargingSchedule) -> jv.Json {
  ...
  let e3 := list.concat(e2, [
    ("chargingRateUnit", JStr(cs.charging_rate_unit)),
    ("chargingSchedulePeriod",
       JList(list.map(cs.charging_schedule_period,
         charging_schedule_period_to_json))),
  ])
...
fn charging_profile_to_json(cp :: ChargingProfile) -> jv.Json {
  let e0 := [
    ("chargingProfileId",      JInt(cp.charging_profile_id)),
  ]
  let e1 := match cp.transaction_id {
    None    => e0,
    Some(t) => list.concat(e0, [("transactionId", JInt(t))]),
  }
  let e2 := list.concat(e1, [
    ("stackLevel",             JInt(cp.stack_level)),
    ("chargingProfilePurpose", JStr(cp.charging_profile_purpose)),
    ("chargingProfileKind",    JStr(cp.charging_profile_kind)),
  ])
  ...
  let e6 := list.concat(e5, [
    ("chargingSchedule", charging_schedule_to_json(cp.charging_schedule)),
  ])
  JObj(e6)
}

...
fn cancel_reservation(message_id :: Str, reservation_id :: Int) -> Str {
  msg.encode(msg.new_call(message_id, a.cancel_reservation(), JObj([("reservationId", JInt(reservation_id))])))
}

# ---- ChangeAvailability -----------------------------------------
fn change_availability(message_id :: Str, connector_id :: Int, avail_type :: Str) -> Str {
  msg.encode(msg.new_call(message_id, a.change_availability(), JObj([("connectorId", JInt(connector_id)), ("type", JStr(avail_type))])))
}

# ---- ChangeConfiguration ----------------------------------------
fn change_configuration(message_id :: Str, key :: Str, value :: Str) -> Str {
  msg.encode(msg.new_call(message_id, a.change_configuration(), JObj([("key", JStr(key)), ("value", JStr(value))])))
}
...
fn clear_charging_profile(message_id :: Str, id :: Option[Int], connector_id :: Option[Int], purpose :: Option[Str], stack_level :: Option[Int]) -> Str {
  let e0 := []
  let e1 := match id {
    None => e0,
...
fn get_composite_schedule(message_id :: Str, connector_id :: Int, duration :: Int, charging_rate_unit :: Option[Str]) -> Str {
  let base := [("connectorId", JInt(connector_id)), ("duration", JInt(duration))]
...
fn get_configuration(message_id :: Str, keys :: List[Str]) -> Str {
  let payload := if list.is_empty(keys) {
    ...
    JObj([("key", JList(list.map(keys, fn (k :: Str) -> jv.Json {
      JStr(k)
    })))])
  }
...
# ---- GetDiagnostics ---------------------------------------------
fn get_diagnostics(message_id :: Str, location :: Str, retries :: Option[Int], retry_interval :: Option[Int], start_time :: Option[Str], stop_time :: Option[Str]) -> Str {
...
fn remote_start_transaction(message_id :: Str, id_tag :: Str, connector_id :: Option[Int]) -> Str {
...
fn remote_stop_transaction(message_id :: Str, transaction_id :: Int) -> Str {
  msg.encode(msg.new_call(message_id, a.remote_stop_transaction(), JObj([("transactionId", JInt(transaction_id))])))
}

# ---- ReserveNow -------------------------------------------------
fn reserve_now(message_id :: Str, connector_id :: Int, expiry_date :: Str, id_tag :: Str, reservation_id :: Int) -> Str {
  msg.encode(msg.new_call(message_id, a.reserve_now(), JObj([("connectorId", JInt(connector_id)), ("expiryDate", JStr(expiry_date)), ("idTag", JStr(id_tag)), ("reservationId", JInt(reservation_id))])))
}

# ---- Reset ------------------------------------------------------
fn reset(message_id :: Str, reset_type :: Str) -> Str {
  msg.encode(msg.new_call(message_id, a.reset(), JObj([("type", JStr(reset_type))])))
}
...
fn send_local_list(message_id :: Str, list_version :: Int, update_type :: Str, local_auth_list :: List[dt.AuthorizationData]) -> Str {
  let base := [("listVersion", JInt(list_version)), ("updateType", JStr(update_type))]
  let fields := if list.is_empty(local_auth_list) {
    base
  } else {
    list.concat(base, [("localAuthorizationList", JList(list.map(local_auth_list, fn (a :: dt.AuthorizationData) -> jv.Json {
...
# ---- SetChargingProfile -----------------------------------------
fn set_charging_profile(message_id :: Str, connector_id :: Int, charging_profile :: dt.ChargingProfile) -> Str {
  msg.encode(msg.new_call(message_id, a.set_charging_profile(), JObj([("connectorId", JInt(connector_id)), ("csChargingProfiles", dt.charging_profile_to_json(charging_profile))])))
}

# ---- TriggerMessage ---------------------------------------------
fn trigger_message(message_id :: Str, requested_message :: Str, connector_id :: Option[Int]) -> Str {
...
fn unlock_connector(message_id :: Str, connector_id :: Int) -> Str {
  msg.encode(msg.new_call(message_id, a.unlock_connector(), JObj([("connectorId", JInt(connector_id))])))
}

# ---- UpdateFirmware ---------------------------------------------
fn update_firmware(message_id :: Str, location :: Str, retrieve_date :: Str, retries :: Option[Int], retry_interval :: Option[Int]) -> Str {
  let e0 := [("location", JStr(location)), ("retrieveDate", JStr(retrieve_date))]
  let e1 := match retries {
    None => e0,
    Some(n) => list.concat(e0, [("retries", JInt(n))]),
  }
...
fn data_transfer(message_id :: Str, vendor_id :: Str, message_id_str :: Option[Str], data :: Option[Str]) -> Str {

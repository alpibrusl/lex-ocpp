...
fn type_call_result() -> Int {
  3
}

fn type_call_error() -> Int {
  4
}
...
type Frame = FrameCall({ message_id :: Str, action :: Str, payload :: jv.Json }) | FrameCallResult({ message_id :: Str, payload :: jv.Json }) | FrameCallError({ message_id :: Str, error_code :: Str, description :: Str, details :: jv.Json })

type FrameError = { code :: Str, message :: Str }
...
fn new_call_error(message_id :: Str, error_code :: Str, description :: Str, details :: jv.Json) -> Frame {
  FrameCallError({ message_id: message_id, error_code: error_code, description: description, details: details })
}
...
fn encode(frame :: Frame) -> Str {
  match frame {
    ...
    FrameCallError(err) => encode_call_error(err.message_id, err.error_code, err.description, err.details),
  }
}

fn encode_call(message_id :: Str, action :: Str, payload :: jv.Json) -> Str {
  jv.stringify(JList([JInt(type_call()), JStr(message_id), JStr(action), payload]))
}

fn encode_call_result(message_id :: Str, payload :: jv.Json) -> Str {
  jv.stringify(JList([JInt(type_call_result()), JStr(message_id), payload]))
}

fn encode_call_error(message_id :: Str, error_code :: Str, description :: Str, details :: jv.Json) -> Str {
  jv.stringify(JList([JInt(type_call_error()), JStr(message_id), JStr(error_code), JStr(description), details]))
}

# ---- Parsing ------------------------------------------------------
fn parse(raw :: Str) -> Result[Frame, FrameError] {
  match jv.parse(raw) {
    Err(p) => Err(frame_err("ProtocolError", str.concat("invalid JSON: ", p.message))),
...
fn parse_frame_json(j :: jv.Json) -> Result[Frame, FrameError] {
  match jv.as_list(j) {
    None => Err(frame_err("ProtocolError", "frame must be a JSON array")),
    Some(xs) => match list.head(xs) {
      None => Err(frame_err("ProtocolError", "frame array is empty")),
      Some(first) => match jv.as_int(first) {
        None => Err(frame_err("ProtocolError", "frame[0] (MessageType) must be a number")),
...
fn dispatch_by_type(t :: Int, rest :: List[jv.Json]) -> Result[Frame, FrameError] {
  if t == type_call() {
    ...
    if t == type_call_result() {
      parse_call_result_rest(rest)
    } else {
      if t == type_call_error() {
        parse_call_error_rest(rest)
      } else {
        Err(frame_err("ProtocolError", str.concat("unknown MessageType ", int.to_str(t))))
      }
    }
  }
}
...
fn parse_call_rest(rest :: List[jv.Json]) -> Result[Frame, FrameError] {
  if list.len(rest) != 3 {
    Err(frame_err("ProtocolError", "Call frame must have 4 elements (type, id, action, payload)"))
  } else {
    ...
    match (mid_o, action_o) {
      (Some(mid_j), Some(action_j)) => match payload_o {
        None => Err(frame_err("ProtocolError", "Call: missing payload")),
...
fn parse_call_typed(mid_j :: jv.Json, action_j :: jv.Json, payload :: jv.Json) -> Result[Frame, FrameError] {
  match jv.as_str(mid_j) {
    None => Err(frame_err("ProtocolError", "Call: message_id must be a string")),
    Some(mid) => match jv.as_str(action_j) {
      None => Err(frame_err("ProtocolError", "Call: action must be a string")),
      Some(action) => match jv.as_obj(payload) {
        None => Err(frame_err("ProtocolError", "Call: payload must be an object")),
...
fn parse_call_result_rest(rest :: List[jv.Json]) -> Result[Frame, FrameError] {
  if list.len(rest) != 2 {
    Err(frame_err("ProtocolError", "CallResult frame must have 3 elements (type, id, payload)"))
  } else {
    let mid_o := list.head(rest)
    let payload_o := list.head(list.tail(rest))
    match (mid_o, payload_o) {
      (Some(mid_j), Some(payload)) => match jv.as_str(mid_j) {
        None => Err(frame_err("ProtocolError", "CallResult: message_id must be a string")),
        Some(mid) => match jv.as_obj(payload) {
          None => Err(frame_err("ProtocolError", "CallResult: payload must be an object")),
...
fn parse_call_error_rest(rest :: List[jv.Json]) -> Result[Frame, FrameError] {
  if list.len(rest) != 4 {
    Err(frame_err("ProtocolError", "CallError frame must have 5 elements"))
  } else {
    let mid_o := list.head(rest)
    ...
    match (mid_o, code_o) {
      (Some(mid_j), Some(code_j)) => match (desc_o, details_o) {
        (Some(desc_j), Some(details)) => parse_call_error_typed(mid_j, code_j, desc_j, details),
...
fn parse_call_error_typed(mid_j :: jv.Json, code_j :: jv.Json, desc_j :: jv.Json, details :: jv.Json) -> Result[Frame, FrameError] {
  match jv.as_str(mid_j) {
    None => Err(frame_err("ProtocolError", "CallError: message_id must be a string")),
    Some(mid) => match jv.as_str(code_j) {
      None => Err(frame_err("ProtocolError", "CallError: error_code must be a string")),
      Some(code) => match jv.as_str(desc_j) {
        None => Err(frame_err("ProtocolError", "CallError: description must be a string")),

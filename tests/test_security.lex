  ...
  match stored {
    ...
    Ok(h) => match pw.verify_argon2id(h, "correct-horse") {
      ...
      Ok(v) => if v {
        ...
        fail("expected verify to return true")
      },
    },
  }
}
...
fn test_wrong_password_does_not_verify() -> Result[Unit, Str] {
  ...
  match stored {
    ...
    Ok(h) => match pw.verify_argon2id(h, "wrong-password") {
      Err(e) => fail(e),
      Ok(v) => if v {
        fail("wrong password should not verify")
      } else {
        pass()
      },
    },
...
fn suite() -> List[Result[Unit, Str]] {
  [test_password_verify_known_hash(), test_wrong_password_does_not_verify(), test_authenticate_ok(), test_authenticate_wrong_pass(), test_issue_and_decode_cp_token()]
}

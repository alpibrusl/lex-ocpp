...
fn test_dispatch_known() -> [io, time, sql] Result[Unit, Str] {
  ...
  let resp := rio.dispatch(reg, msg.new_call("42", "Heartbeat", JObj([])))
  match resp {
...
fn test_dispatch_unknown() -> [io, time, sql] Result[Unit, Str] {
  let reg := rio.new()
  let resp := rio.dispatch(reg, msg.new_call("42", "Mystery", JObj([])))
  match resp {
...
fn test_schema_gate_rejects() -> [io, time, sql] Result[Unit, Str] {
  let reg := rio.handler_with_schema(rio.new(), "Authorize", require_id_tag, echo_handler)
  let resp := rio.dispatch(reg, msg.new_call("42", "Authorize", JObj([])))
  match resp {
...
fn test_schema_gate_accepts() -> [io, time, sql] Result[Unit, Str] {
  let reg := rio.handler_with_schema(rio.new(), "Authorize", require_id_tag, echo_handler)
  let resp := rio.dispatch(reg, msg.new_call("42", "Authorize", JObj([("idTag", JStr("ABC"))])))
  match resp {
...
fn test_actions_list() -> [io, time, sql] Result[Unit, Str] {
  let reg := rio.handler(rio.handler(rio.new(), "Heartbeat", pong_handler), "Authorize", echo_handler)
  if list.len(rio.actions(reg)) == 2 {
    pass()
  } else {
    fail("expected 2 actions")
  }
}

# ---- Suite + runner ---------------------------------------------
fn suite() -> [io, time, sql] List[Result[Unit, Str]] {
  [test_dispatch_known(), test_dispatch_unknown(), test_handler_returns_error(), test_schema_gate_rejects(), test_schema_gate_accepts(), test_handle_raw_round_trip(), test_actions_list()]
}

...
fn test_boot_ok() -> Result[Unit, Str] {
  let payload := JObj([("chargePointVendor", JStr("ACME")), ("chargePointModel", JStr("Model-X")), ("firmwareVersion", JStr("1.0.0"))])
  assert_ok(sch.validate_boot_notification_req(payload), "boot_ok")
}

fn test_boot_missing_vendor() -> Result[Unit, Str] {
  let payload := JObj([("chargePointModel", JStr("Model-X"))])
  assert_err(sch.validate_boot_notification_req(payload), "boot_missing_vendor")
}

fn test_boot_vendor_too_long() -> Result[Unit, Str] {
  let payload := JObj([("chargePointVendor", JStr("ThisVendorNameIsWayTooLongForOCPP")), ("chargePointModel", JStr("Model-X"))])
...
fn test_status_notification_ok() -> Result[Unit, Str] {
  let payload := JObj([("connectorId", JInt(1)), ("errorCode", JStr(en.ec_no_error())), ("status", JStr(en.cp_available()))])
  assert_ok(sch.validate_status_notification_req(payload), "status_ok")
}

fn test_status_notification_bad_enum() -> Result[Unit, Str] {
  let payload := JObj([("connectorId", JInt(1)), ("errorCode", JStr(en.ec_no_error())), ("status", JStr("UnknownStatus"))])
  assert_err(sch.validate_status_notification_req(payload), "status_bad_enum")
}

# ---- StartTransaction.req ---------------------------------------
fn test_start_transaction_ok() -> Result[Unit, Str] {
  let payload := JObj([("connectorId", JInt(1)), ("idTag", JStr("USER-001")), ("meterStart", JInt(0)), ("timestamp", JStr("2026-05-13T12:00:00Z"))])
  assert_ok(sch.validate_start_transaction_req(payload), "start_tx_ok")
}

fn test_start_transaction_zero_connector() -> Result[Unit, Str] {
  let payload := JObj([("connectorId", JInt(0)), ("idTag", JStr("USER-001")), ("meterStart", JInt(0)), ("timestamp", JStr("2026-05-13T12:00:00Z"))])
  assert_err(sch.validate_start_transaction_req(payload), "start_tx_zero_connector")
}

# ---- StopTransaction.req ----------------------------------------
fn test_stop_transaction_ok() -> Result[Unit, Str] {
  let payload := JObj([("meterStop", JInt(1500)), ("timestamp", JStr("2026-05-13T13:00:00Z")), ("transactionId", JInt(12345)), ("reason", JStr(en.reason_local()))])
  assert_ok(sch.validate_stop_transaction_req(payload), "stop_tx_ok")
}

fn test_stop_transaction_bad_reason() -> Result[Unit, Str] {
  let payload := JObj([("meterStop", JInt(1500)), ("timestamp", JStr("2026-05-13T13:00:00Z")), ("transactionId", JInt(12345)), ("reason", JStr("AlienAbduction"))])
  assert_err(sch.validate_stop_transaction_req(payload), "stop_tx_bad_reason")
}

# ---- MeterValues.req --------------------------------------------
fn test_meter_values_ok() -> Result[Unit, Str] {
  let sv := JObj([("value", JStr("3500")), ("measurand", JStr(en.ms_energy_active_import_register())), ("unit", JStr(en.uom_wh()))])
  let mv := JObj([("timestamp", JStr("2026-05-13T12:30:00Z")), ("sampledValue", JList([sv]))])
  let payload := JObj([("connectorId", JInt(1)), ("meterValue", JList([mv]))])
  assert_ok(sch.validate_meter_values_req(payload), "meter_values_ok")
}

fn test_meter_values_empty_list() -> Result[Unit, Str] {
  let payload := JObj([("connectorId", JInt(1)), ("meterValue", JList([]))])
  assert_err(sch.validate_meter_values_req(payload), "meter_values_empty")
}

# ---- DataTransfer.req -------------------------------------------
fn test_data_transfer_ok() -> Result[Unit, Str] {
  let payload := JObj([("vendorId", JStr("com.example.vendor")), ("messageId", JStr("foo.bar")), ("data", JStr("payload"))])
...
fn test_reset_ok() -> Result[Unit, Str] {
  assert_ok(sch.validate_reset_req(JObj([("type", JStr(en.reset_hard()))])), "reset_ok")
}

fn test_reset_bad_type() -> Result[Unit, Str] {
  assert_err(sch.validate_reset_req(JObj([("type", JStr("Nuclear"))])), "reset_bad")
}
...
fn suite() -> List[Result[Unit, Str]] {
  [test_boot_ok(), test_boot_missing_vendor(), test_boot_vendor_too_long(), test_authorize_ok(), test_authorize_empty_id_tag(), test_authorize_missing_id_tag(), test_heartbeat_ok(), test_status_notification_ok(), test_status_notification_bad_enum(), test_start_transaction_ok(), test_start_transaction_zero_connector(), test_stop_transaction_ok(), test_stop_transaction_bad_reason(), test_meter_values_ok(), test_meter_values_empty_list(), test_data_transfer_ok(), test_data_transfer_missing_vendor(), test_firmware_status_ok(), test_firmware_status_bad_enum(), test_reset_ok(), test_reset_bad_type(), test_find_validator_known(), test_find_validator_unknown()]
}

...
fn test_v16_uses_occurence() -> Result[Unit, Str] {
  assert_true(list_contains(oe.all_v16(), "OccurenceConstraintViolation"),
    "v1.6 should expose OccurenceConstraintViolation (the spec's typo)")
}

fn test_v201_uses_occurrence() -> Result[Unit, Str] {
  assert_true(list_contains(oe.all_v201(), "OccurrenceConstraintViolation"),
    "v2.0.1 should expose OccurrenceConstraintViolation (the corrected spelling)")
}

fn test_v201_uses_format_violation() -> Result[Unit, Str] {
  assert_true(list_contains(oe.all_v201(), "FormatViolation"),
    "v2.0.1 should expose FormatViolation (replaces FormationViolation)")
}
...
fn test_not_implemented_err() -> Result[Unit, Str] {
  ...
  if e.code == "NotImplemented" {
    assert_eq_str("action not implemented: MysteryAction",
...
fn test_from_schema_errors_carries_details() -> Result[Unit, Str] {
  let oerr := oe.from_schema_errors([
    { path: "idTag", code: "min_len", message: "must be at least 1 characters" },
  ])
...
fn suite() -> List[Result[Unit, Str]] {
  [
    test_all_v16_count(),
    test_all_v201_count(),
    test_v16_uses_occurence(),
    test_v201_uses_occurrence(),
    test_v201_uses_format_violation(),
    test_not_implemented_constant(),
    test_protocol_error_constant(),
    test_not_implemented_err(),
    test_from_schema_errors_empty(),
    test_from_schema_errors_carries_details(),
  ]
}

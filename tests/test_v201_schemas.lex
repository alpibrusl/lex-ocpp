...
fn test_cancel_reservation_ok() -> Result[Unit, Str] {
  assert_ok_validation(
    sch.validate_cancel_reservation_req(JObj([("reservationId", JInt(7))])),
    "cancel_reservation")
}

fn test_cancel_reservation_missing() -> Result[Unit, Str] {
  assert_err_validation(
    sch.validate_cancel_reservation_req(JObj([])),
    "cancel_reservation missing")
}

fn test_get_variables_ok() -> Result[Unit, Str] {
  let item := JObj([
    ("component", JObj([("name", JStr("AlignedDataCtrlr"))])),
    ("variable",  JObj([("name", JStr("Enabled"))])),
  ])
  let payload := JObj([("getVariableData", JList([item]))])
  assert_ok_validation(
    sch.validate_get_variables_req(payload), "get_variables")
}

fn test_get_variables_empty_list() -> Result[Unit, Str] {
  let payload := JObj([("getVariableData", JList([]))])
  assert_err_validation(
    sch.validate_get_variables_req(payload), "get_variables empty")
}

fn test_set_variables_ok() -> Result[Unit, Str] {
  let item := JObj([
    ("attributeValue", JStr("true")),
    ("component", JObj([("name", JStr("AlignedDataCtrlr"))])),
    ("variable",  JObj([("name", JStr("Enabled"))])),
  ])
  let payload := JObj([("setVariableData", JList([item]))])
  assert_ok_validation(
    sch.validate_set_variables_req(payload), "set_variables")
}

fn test_get_base_report_ok() -> Result[Unit, Str] {
  let payload := JObj([
    ("requestId",  JInt(1)),
    ("reportBase", JStr("FullInventory")),
  ])
  assert_ok_validation(
    sch.validate_get_base_report_req(payload), "get_base_report")
}

fn test_get_base_report_bad_enum() -> Result[Unit, Str] {
  let payload := JObj([
    ("requestId",  JInt(1)),
    ("reportBase", JStr("WhateverInventory")),
  ])
  assert_err_validation(
    sch.validate_get_base_report_req(payload), "get_base_report bad")
}

fn test_log_status_notification_ok() -> Result[Unit, Str] {
  let payload := JObj([("status", JStr("Idle"))])
  assert_ok_validation(
    sch.validate_log_status_notification_req(payload), "log_status")
}

fn test_sign_certificate_ok() -> Result[Unit, Str] {
  let payload := JObj([
    ("csr", JStr("-----BEGIN CERTIFICATE REQUEST-----\nMIIB...")),
    ("certificateType", JStr("ChargingStationCertificate")),
  ])
  assert_ok_validation(
    sch.validate_sign_certificate_req(payload), "sign_certificate")
}

fn test_update_firmware_ok() -> Result[Unit, Str] {
  let firmware := JObj([
    ("location",         JStr("https://updates.example.com/fw-1.2.3.bin")),
    ("retrieveDateTime", JStr("2026-05-13T12:00:00Z")),
  ])
  let payload := JObj([
    ("requestId", JInt(99)),
    ("firmware",  firmware),
  ...
  assert_ok_validation(
    sch.validate_update_firmware_req(payload), "update_firmware")
}
...
fn test_notify_event_ok() -> Result[Unit, Str] {
  let event := JObj([
    ("eventId",               JInt(1)),
    ("timestamp",             JStr("2026-05-13T12:00:00Z")),
    ("trigger",               JStr("Periodic")),
    ("actualValue",           JStr("42")),
    ("eventNotificationType", JStr("HardWiredNotification")),
    ("component", JObj([("name", JStr("AlignedDataCtrlr"))])),
    ("variable",  JObj([("name", JStr("Enabled"))])),
  ])
  let payload := JObj([
    ("generatedAt", JStr("2026-05-13T12:00:00Z")),
    ("seqNo",       JInt(0)),
    ("eventData",   JList([event])),
  ])
  assert_ok_validation(
    sch.validate_notify_event_req(payload), "notify_event")
}
...
fn suite() -> List[Result[Unit, Str]] {
  [
    ...
    test_clear_cache_ok(),
    test_cancel_reservation_ok(),
    test_cancel_reservation_missing(),
    test_get_variables_ok(),
    test_get_variables_empty_list(),
    test_set_variables_ok(),
    test_get_base_report_ok(),
    test_get_base_report_bad_enum(),
    test_log_status_notification_ok(),
    test_sign_certificate_ok(),
    test_update_firmware_ok(),
    test_notify_event_ok(),
  ]
}

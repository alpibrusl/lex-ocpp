...
fn test_dispatch_with_schema_accepts_valid_payload() -> Result[Unit, Str] {
  ...
  let resp := route.dispatch(reg,
    msg.new_call("42", "Authorize", payload))
  match resp {
...
fn test_passthrough_non_call() -> Result[Unit, Str] {
  ...
  if msg.is_call_result(resp) { pass() }
  else { fail("expected non-Call to pass through") }
}
...
fn test_actions_list() -> Result[Unit, Str] {
  let reg := route.handler(
    route.handler(route.new(), "Heartbeat", pong_handler),
    "Authorize", echo_handler)
  let actions := route.actions(reg)
  if list.len(actions) == 2 { pass() }
  else { fail("expected 2 registered actions") }
}
...
fn suite() -> List[Result[Unit, Str]] {
  [
    test_dispatch_known_action(),
    test_dispatch_unknown_action_returns_not_implemented(),
    test_dispatch_handler_can_return_error(),
    test_dispatch_with_schema_rejects_bad_payload(),
    test_dispatch_with_schema_accepts_valid_payload(),
    test_passthrough_non_call(),
    test_handle_raw_round_trip(),
    test_handle_raw_bad_input(),
    test_actions_list(),
  ]
}

...
fn test_encode_call() -> Result[Unit, Str] {
  ...
  assert_eq_str("[2,\"19223201\",\"BootNotification\",{}]", msg.encode(f), "encode_call")
}

fn test_encode_call_result() -> Result[Unit, Str] {
  let f := msg.new_call_result("19223201", JObj([("status", JStr("Accepted"))]))
  assert_eq_str("[3,\"19223201\",{\"status\":\"Accepted\"}]", msg.encode(f), "encode_call_result")
}

fn test_encode_call_error() -> Result[Unit, Str] {
  let f := msg.new_call_error("19223201", "NotImplemented", "Unknown action", JObj([]))
  assert_eq_str("[4,\"19223201\",\"NotImplemented\",\"Unknown action\",{}]", msg.encode(f), "encode_call_error")
}
...
fn test_parse_unknown_message_type() -> Result[Unit, Str] {
  match msg.parse("[9,\"id\",\"X\",{}]") {
    Err(e) => assert_true(str.contains(e.message, "unknown MessageType"), "expected 'unknown MessageType' error"),
...
fn suite() -> List[Result[Unit, Str]] {
  [test_encode_call(), test_encode_call_result(), test_encode_call_error(), test_parse_call(), test_parse_call_result(), test_parse_call_error(), test_parse_invalid_json(), test_parse_wrong_type(), test_parse_unknown_message_type(), test_parse_call_wrong_arity(), test_parse_call_non_string_id(), test_round_trip_call()]
}
